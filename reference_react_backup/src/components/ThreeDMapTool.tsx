...
const ThreeDMapTool: React.FC<ThreeDMapToolProps> = ({ isActive }) => {
  ...
  useEffect(() => {
    ...
    const mmMap = new maplibregl.Map({
      ...
      bearing: 0,
      attributionControl: false,
      interactive: false // We will handle interactions manually on the div
    });
    ...
    mlMap.on('load', () => {
      ...
      if (mlMap.touchZoomRotate) mlMap.touchZoomRotate.disableRotation(); // Keep rotate, but prevent zoom? Actually disable touchZoomRotate completely, maybe dragRotate is enough.
  ...
  const handleMinimapTap = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // If expanded, tapping the minimap re-centers the 3D map
    const rect = minimapContainerRef.current!.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    
    const lngLat = minimapRef.current.unproject([x, y]);
    mlMapRef.current.flyTo({ center: lngLat, zoom: mlMapRef.current.getZoom(), duration: 800 });
  };
